...
def setup_chain(selected_subject, selected_chapter):
...
st.set_page_config(
    page_title="St. Mary's Study Chatbot",
    page_icon="🌀",
    layout="centered"
)

st.title("St. Mary's Study Chatbot")
...
if selected_subject:
    ...
    selected_chapter= st.selectbox(
        label= f"Select a Chapter from {selected_subject}",
        options= chapter_list,
        index=0
    )
    if selected_chapter:
        if st.session_state.get("selected_chapter")!=selected_chapter:
            st.session_state.chat_chain= setup_chain(selected_subject, selected_chapter)
